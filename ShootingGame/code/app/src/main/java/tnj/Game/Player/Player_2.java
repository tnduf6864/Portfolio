    ...
    public void Attack() {
        if(playerShootLevel == 1) {
            ...
            if(System.currentTimeMillis() - LastShoot >= 550) {
                LastShoot = System.currentTimeMillis();
                SoundManager.getInstance().play(1);
                AppManager.getInstance().getGameState1().m_pmslist.add(new Missile_Player(m_x, m_y - 70, 2));
                AppManager.getInstance().getGameState1().m_pmslist.add(new Missile_Player(m_x, m_y - 70, 3));
            }
        }
