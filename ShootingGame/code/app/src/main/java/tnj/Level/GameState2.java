...
import tnj.Game.Enemy.Enemy_5;
import tnj.Game.Enemy.Enemy_6;
...
public class GameState2 implements IState {
    ...
    public int skillCount = AppManager.getInstance().getBossState1().skillCount; // 남은 스킬횟수를 가져옴
    ...
    public GameState2() {AppManager.getInstance().setGameState2(this);
    }
    @Override
    public void Destroy() {
    ...
    public void Render(Canvas canvas) {
        ...
        if(System.currentTimeMillis() - lastStage<3000)
            canvas.drawText("STAGE 2",400,400,p1);
    ...
    public void Update() {
        ...
        if(System.currentTimeMillis() - lastStage>30000) {
            ...
            AppManager.getInstance().getGameView().changeGameState(new BossState2());
        }
            makeEnemy();
        CheckCollision();
        CheckHP();
