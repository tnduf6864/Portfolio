...
public class BossState3 implements IState {
    private Boss_3 m_boss;
    public ArrayList<Missile_Boss> m_bossmslist = new ArrayList<>();
    private BackGround m_background;
    public int skillCount = AppManager.getInstance().getGameState3().skillCount; // 남은 스킬횟수를 가져옴
    Paint p1 = new Paint();

    Random randEnem = new Random();
    long lastStage = System.currentTimeMillis();
    long lastRegenMissile = System.currentTimeMillis();
    public BossState3(){AppManager.getInstance().setBossState3(this); }
    ...
    public void Init() {
        m_boss = new Boss_3();
        m_background =AppManager.getInstance().getGameState3().m_background;
        p1.setTextSize(80);
