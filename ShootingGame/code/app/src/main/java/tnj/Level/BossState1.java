...
public class BossState1 implements IState {
    private Boss_1 m_boss;
    public ArrayList<Missile_Boss> m_bossmslist = new ArrayList<>();
    private BackGround m_background;
    public int skillCount = AppManager.getInstance().getGameState1().skillCount; // 남은 스킬횟수를 가져옴
    Paint p1 = new Paint();

    long lastStage = System.currentTimeMillis();
    public BossState1(){ AppManager.getInstance().setBossState1(this); }
    ...
    public void Init() {
        m_boss = new Boss_1();
        m_background =AppManager.getInstance().getGameState1().m_background;
        p1.setTextSize(80);
    ...
    public void Update() {
       ...
       if(m_boss.state == m_boss.STATE_OUT){
           SoundManager.getInstance().pauseSound(1);
            ...
            AppManager.getInstance().getGameView().changeGameState(new GameState2());
        }
    }
    @Override
